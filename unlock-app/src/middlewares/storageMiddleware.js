...
import { UPDATE_LOCK, updateLock, addLock } from '../actions/lock'
...
const storageMiddleware = config => {
  ...
  return ({ getState, dispatch }) => {
    ...
    storageService.on(failure.getLockAddressesForUser, () => {
      // Not a problem because we will get them thru the transactions?
    })
    ...
    return next => {
      return action => {
        ...
        if (action.type === SET_ACCOUNT) {
          ...
          // Wehn we set the account, we want to retrive the list of locks
